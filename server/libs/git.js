...
module.exports = {
  ...
  _initRepo(appconfig) {
    ...
    return fs.mkdirAsync(self._repo.path).catch((err) => {
      ...
      let gitConfigs = [
        () => { return self._git.exec('config', ['--local', 'user.name', 'IOTA Wiki']) },
  ...
  commitDocument(entryPath, author) {
    ...
    return self._git.exec('ls-files', gitFilePath).then((cProc) => {
      ...
      return self._git.exec('commit', ['-m', commitMsg.replace(/&#x2F;/g, '/'), '--author="' + commitUsr.name + ' <' + commitUsr.email + '>"']).catch((err) => {
  ...
  commitUploads(msg) {
    ...
    return self._git.add('uploads').then(() => {
      return self._git.commit(msg.replace(/&#x2F;/g, '/')).catch((err) => {
        if (_.includes(err.stdout, 'nothing to commit')) { return true }
      })
    })
  },
